...
Here, we are using a sample of the population, which we do not know much about, to make inferences in the population. Specifically, we are trying to measure whether there are differences in means from Congruent and Incongruent in the population.   

This is a dependent test, where the same sample is tested pre and post treatment.

The Null Hypothesis would be that sample mean performance time for the Congruent sample are equal to mean performance time in the Incongruent sample.

The Alternative Hypothesis would be that the mean performance time of the Incongruent sample is not equal to the mean performance time of the Congruent sample.
...
Let's now run the data through a t-test. We will use a 95% Confidence level, which with 46 degrees of freedom translates a t critical value of +/- 2.069. 
...
With t-statistic (-8.02) < t-critical (+/- 2.069), p < .05, and 24 degrees of freedom, we reject the null hypothesis.The 95% Confidence Interval is (-10.02, -5.91). The evidence suggests that there is worse performance when undergoing the incongruent treatment than the congruent treatment. 
